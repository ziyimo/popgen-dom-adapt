...
DataGen = GTmaskedSequence(f"{SRC_pref}_train.npz", f"{TAR_pref}_train.npz", batch_size)

# Load validation data
with np.load(f"{TAR_pref}_val.npz") as val_npz:    
...
val_Y_discr = -1*np.ones(len(val_rho))

print("GTM_VAL\tPOS_VAL\tRHO_VAL", flush=True)
print(val_bgtm.shape, val_pos.shape, val_rho.shape, val_Y_discr.shape, flush=True)

print("TRAIN_NO_BATCH", flush=True)
print(DataGen.no_batch, flush=True)

if CONT == "new":
  print(">>>CREATE NEW MODEL", flush=True)
  # mirrored_strategy = tf.distribute.MirroredStrategy()
  # with mirrored_strategy.scope():
  merged_model = ReLERNN_DA()
else:
  print(f">>>LOAD FROM EXISTING MODEL: {CONT}", flush=True)
  merged_model = tf.keras.models.load_model(CONT)

## Callbacks ##
erly_stp = keras.callbacks.EarlyStopping(monitor='val_predictor_loss', patience=50)
...
history = merged_model.fit(DataGen, validation_data=([val_bgtm, val_pos], {"predictor":val_rho, "discriminator":val_Y_discr}),
